...
import com.emin.platform.wxbase.interfaces.basic.KeyReplyApiFeign;

@Controller
@RequestMapping("/keyReply")
public class KeyReplyController  extends BaseController {
	@Autowired
	WxoaApiFeign wxoaApiFeign;
	
	@Autowired
	KeyReplyApiFeign keyReplyApiFeign;
	
	@RequestMapping("/index")
	@ResponseBody
	public ModelAndView goManage(Long woaId, String keyword){
		ModelAndView mv = new ModelAndView("modules/keyReply/manage");
		Long ecmId = null;
		...
		if (woaId != null) {
			...
			String res = keyReplyApiFeign.page(ecmId, woaId, getPageRequestData().getCurrentPage(), getPageRequestData().getLimit(), keyword);
			if (res != null) {
	...
	public ModelAndView goForm(Long woaId, Long id){
		ModelAndView mv = new ModelAndView("modules/keyReply/form");
		if (woaId != null) {
			mv.addObject("woaId", woaId);
			if (id != null) {
				Long ecmId = null;
				...
				if (ecmId == null) {
					...
					String res = keyReplyApiFeign.detail(ecmId, id);
					if (res != null) {
						JSONObject resJson = JSONObject.parseObject(res);
						if (!resJson.getBooleanValue("success")) {
							throw new EminException(resJson.getString("code"));
						}
						mv.addObject("keyReply", resJson.get("result"));
					}
				}
			}
		}
	...
	public JSONObject save(String jsonStr, String articleIds){
		...
		if (ecmId != null) {
			String res = keyReplyApiFeign.save(ecmId, jsonStr, articleIds);
			if (res != null) {
	...
	public JSONObject delete(Long ids){
		...
		if (ecmId != null) {
			String res = keyReplyApiFeign.delete(ecmId, ids);
			if (res != null) {
				json = JSONObject.parseObject(res);
				if (!json.getBooleanValue("success")) {
					throw new EminException(json.getString("code"));
